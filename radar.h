...
0x0		按发送目标数过滤，最小值默认为0,最大128
0x1		按径向距离过滤(0--409m)
0x2		按水平角度过滤(-102°--102°)
0x3		按靠近速度的绝对值过滤(0--102m/s)
0x4		按远离速度的绝对值过滤(0--102m/s)
0x5		按RCS的绝对值过滤(-50--52dbsm)
0x6		按信噪比过滤(0--102db)
0x7		按X坐标过滤(-204--204m)
0x8		按Y坐标过滤(0--409m)
0x9		对靠近的目标按vy绝对值过滤，保留所有远离目标(0--102m/s)
0xa		对远离的目标按vy绝对值过滤，保留所有靠近目标(0--102m/s)
0xb		对从右到左运动的目标，按vx绝对值过滤，保留所有从左到右的目标(0--102m/s)
0xc		对从左到右运动的目标，按vy绝对值过滤，保留所有从右到左的目标(0--102m/s)
）
